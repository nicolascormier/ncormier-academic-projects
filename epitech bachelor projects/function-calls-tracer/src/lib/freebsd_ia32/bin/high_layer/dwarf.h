...
typedef struct		_Dwarf2_CUH
{
  unsigned int		length;
  unsigned short	version;
  unsigned int		abbrevoff;
  unsigned char		addrsize;
}			__attribute__((packed)) Dwarf2_CUH;

typedef struct		_Dwarf2_SP
{
  unsigned int		length;
  unsigned short	version;
...
}			__attribute__((packed)) Dwarf2_SP;

typedef struct	s_dwarf
{
  unsigned char	*d_dinfo;
  int		d_dinfosize;
  unsigned char	*d_dabbrev;
  int		d_dabbrevsize;
  unsigned char	*d_dstr;
  int		d_dstrsize;
  unsigned char	*d_dline;
  int		d_dlinesize;
  Dwarf2_CUH	*d_cuh;
  int		d_cuhoff;
  Dwarf2_SP	*d_sp;
...
typedef struct	s_forminfo
{
  char		f_type;
# define FORMTYPE_INT		1
# define FORMTYPE_LONGLONG	2
# define FORMTYPE_CHAR		3
# define FORMTYPE_STRING	4
# define FORMTYPE_BLOCK1	5
# define FORMTYPE_SHORT		6
  int		f_ivalue;
  long long	f_llvalue;
  unsigned char	f_cvalue;
  short		f_shvalue;
  unsigned char	*f_svalue;
  char		f_b1value[255];
}		t_forminfo;
